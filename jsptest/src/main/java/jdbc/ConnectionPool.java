...
public final class ConnectionPool{
	//ConnectionPool 객체가 생성될때 static이 실행된다.
	static{
	try{
    ...
    //사용하지 않은 커넥션 즉, 초기 커넥션을 저장하는 멤버 변수
    private Vector<Connection> free; 
    private Vector<Connection> used; //사용중인 커넥션을 저장하는 변수
    ...
    //싱글톤 방식 1 (외부에서 new 접근 불가) 자기를 참조하는게 하나 더 있어야된다.
    //싱글톤에서 생성자 private -> 자기께 와야된다(static)
    private static ConnectionPool cp;//cp 객체 참조 변수
		...
		try{
			if(cp == null){
				synchronized(ConnectionPool.class){
				   cp=new ConnectionPool() ;//ConnectionPool 객채를 만들어서 주기
				}
			}
		}
